...
void
Level::load(string fileName, Player& player)
{
  ifstream file;
  file.open( fileName );
  if (file.fail()) {
...
void
Level::print(Player& player)
{
  clear();
  //TODO separate map printing from HUD printing
  //TODO make focus on player in the center
...
void
Level::movePlayer(char input, Player& player)
{
  int playerX;
  int playerY;
...
void
Level::updateEnemies(Player& player)
{
  int playerX;
  int playerY;
  int enemyX;
...
void
Level::setTile(int x, int y, char tile)
{
  _levelData[y][x] = tile;
}

void
Level::processPlayerMove(Player& player, int targetX, int targetY)
{
  int playerX;
  int playerY;
...
void
Level::processEnemyMove(Player& player, int enemyIndex, int targetX, int targetY)
{
  int playerX;
  int playerY;
  int enemyX;
...
void
Level::battleMonster(Player& player, int targetX, int targetY)
{
  int enemyX;
  int enemyY;
  int playerX;
  ...
  //TODO print monster's HP
  //TODO make sleep instead of cin.get()
  player.getPosition( playerX, playerY );
  for (int i = 0; i < _enemies.size(); ++i) {
    _enemies[i].getPosition( enemyX, enemyY );
